...
class Tokens {
	...
	public function get( $data, $token_api_url ) {
		...
		if ( ! $role ) {
			return new WP_Error( 'role', __( 'An administrator for this blog must set up the Jetpack connection.', 'woocommerce' ) );
		}

		$client_secret = $this->get_access_token();
		if ( ! $client_secret ) {
			return new WP_Error( 'client_secret', __( 'You need to register your Jetpack before connecting it.', 'woocommerce' ) );
		}
		...
		*/
		...
		if ( 200 !== $code || ! empty( $json->error ) ) {
			...
			$error_description = isset( $json->error_description ) ? sprintf( __( 'Error Details: %s', 'woocommerce' ), (string) $json->error_description ) : '';
	...
	public function get_access_token( $user_id = false, $token_key = false, $suppress_errors = true ) {
		...
		if ( $user_id ) {
			if ( ! $user_tokens ) {
				return $suppress_errors ? false : new WP_Error( 'no_user_tokens', __( 'No user tokens found', 'woocommerce' ) );
			}
			if ( true === $user_id ) { // connection owner.
				$user_id = Jetpack_Options::get_option( 'master_user' );
				if ( ! $user_id ) {
					return $suppress_errors ? false : new WP_Error( 'empty_master_user_option', __( 'No primary user defined', 'woocommerce' ) );
				}
			}
			if ( ! isset( $user_tokens[ $user_id ] ) || ! $user_tokens[ $user_id ] ) {
				// translators: %s is the user ID.
				return $suppress_errors ? false : new WP_Error( 'no_token_for_user', sprintf( __( 'No token for user %d', 'woocommerce' ), $user_id ) );
			}
			$user_token_chunks = explode( '.', $user_tokens[ $user_id ] );
			if ( empty( $user_token_chunks[1] ) || empty( $user_token_chunks[2] ) ) {
				// translators: %s is the user ID.
				return $suppress_errors ? false : new WP_Error( 'token_malformed', sprintf( __( 'Token for user %d is malformed', 'woocommerce' ), $user_id ) );
			}
			if ( $user_token_chunks[2] !== (string) $user_id ) {
				// translators: %1$d is the ID of the requested user. %2$d is the user ID found in the token.
				return $suppress_errors ? false : new WP_Error( 'user_id_mismatch', sprintf( __( 'Requesting user_id %1$d does not match token user_id %2$d', 'woocommerce' ), $user_id, $user_token_chunks[2] ) );
			}
		...
		if ( ! $possible_tokens ) {
			...
			return $suppress_errors ? false : new WP_Error( 'no_possible_tokens', __( 'No blog token found', 'woocommerce' ) );
		}
		...
		if ( ! $valid_token ) {
			if ( $user_id ) {
				...
				return $suppress_errors ? false : new WP_Error( 'no_valid_user_token', sprintf( __( 'Invalid token for user %d', 'woocommerce' ), $user_id ) );
			} else {
				return $suppress_errors ? false : new WP_Error( 'no_valid_blog_token', __( 'Invalid blog token', 'woocommerce' ) );
			}
		}
