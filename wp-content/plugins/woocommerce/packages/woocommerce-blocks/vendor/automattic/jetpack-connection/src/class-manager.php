...
class Manager {
	...
	public function update_connection_owner( $new_owner_id ) {
		...
		if ( ! user_can( $new_owner_id, $roles->translate_role_to_cap( 'administrator' ) ) ) {
			return new WP_Error(
				'new_owner_not_admin',
				__( 'New owner is not admin', 'woocommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $old_owner_id === $new_owner_id ) {
			return new WP_Error(
				...
				__( 'New owner is same as existing owner', 'woocommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $this->is_user_connected( $new_owner_id ) ) {
			return new WP_Error(
				...
				__( 'New owner is not connected', 'woocommerce' ),
				array( 'status' => 400 )
			);
		}
		...
		return new WP_Error(
			...
			__( 'Could not confirm new owner.', 'woocommerce' ),
			array( 'status' => 500 )
		);
	}
	...
	public function register( $api_endpoint = 'register' ) {
		...
		if ( false === $secrets ) {
			return new WP_Error( 'cannot_save_secrets', __( 'Jetpack experienced an issue trying to save options (cannot_save_secrets). We suggest that you contact your hosting provider, and ask them for help checking that the options table is writable on your site.', 'woocommerce' ) );
		}
	...
	protected function validate_remote_register_response( $response ) {
		...
		if ( 5 === $code_type ) {
			...
			if (
				...
				$error_description = __( "PHP's XML extension is not available. Jetpack requires the XML extension to communicate with WordPress.com. Please contact your hosting provider to enable PHP's XML extension.", 'woocommerce' );
			} else {
		...
		if ( empty( $registration_response->jetpack_id ) ) {
			return new \WP_Error(
				...
				sprintf( __( 'Error Details: Jetpack ID is empty. Do not publicly post this error message! %s', 'woocommerce' ), $entity ),
				$entity
			);
			...
			return new \WP_Error(
				'jetpack_id',
				/* translators: %s is an error message string */
				sprintf( __( 'Error Details: Jetpack ID is not a scalar. Do not publicly post this error message! %s', 'woocommerce' ), $entity ),
				$entity
			);
			...
			return new \WP_Error(
				'jetpack_id',
				/* translators: %s is an error message string */
				sprintf( __( 'Error Details: Jetpack ID begins with a numeral. Do not publicly post this error message! %s', 'woocommerce' ), $entity ),
				$entity
			);
		}
	...
	public function handle_registration( array $registration_data ) {
		...
		if ( empty( $registration_user_id ) ) {
			return new \WP_Error( 'registration_state_invalid', __( 'Invalid Registration State', 'woocommerce' ), 400 );
		}
	...
	public function is_usable_domain( $domain ) {
		...
		if ( ! $domain ) {
			return new \WP_Error(
				...
				sprintf( __( 'Domain `%1$s` just failed is_usable_domain check as it is empty.', 'woocommerce' ), $domain )
			);
		}
	...
	public function jetpack_get_options( $args ) {
		...
		if ( is_array( $user_data ) ) {
			$options['jetpack_user_id']         = array(
				'desc'     => __( 'The WP.com user ID of the connected user', 'woocommerce' ),
				'readonly' => true,
				'value'    => $user_data['ID'],
			);
			$options['jetpack_user_login']      = array(
				'desc'     => __( 'The WP.com username of the connected user', 'woocommerce' ),
				'readonly' => true,
				'value'    => $user_data['login'],
			);
			$options['jetpack_user_email']      = array(
				'desc'     => __( 'The WP.com user email of the connected user', 'woocommerce' ),
				'readonly' => true,
				'value'    => $user_data['email'],
			);
			$options['jetpack_user_site_count'] = array(
				'desc'     => __( 'The number of sites of the connected WP.com user', 'woocommerce' ),
				'readonly' => true,
	...
	public function xmlrpc_options( $options ) {
		...
		$options['jetpack_version'] = array(
			'desc'     => __( 'Jetpack Plugin Version', 'woocommerce' ),
			'readonly' => true,
			'value'    => Constants::get_constant( 'JETPACK__VERSION' ),
		);

		$options['jetpack_client_id'] = array(
			'desc'     => __( 'The Client ID/WP.com Blog ID of this site', 'woocommerce' ),
			'readonly' => true,
	...
	public function refresh_blog_token() {
		...
		if ( 200 !== $code ) {
			...
			$error_description = isset( $json->message ) ? sprintf( __( 'Error Details: %s', 'woocommerce' ), (string) $json->message ) : '';
	...
	public static function get_site_id() {
		...
		if ( ! $site_id ) {
			return new \WP_Error(
				'unavailable_site_id',
				__( 'Sorry, something is wrong with your Jetpack connection.', 'woocommerce' ),
				403
			);
		}
		return (int) $site_id;
