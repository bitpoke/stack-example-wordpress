		...
		if ( empty( $secret_1 ) ) {
			$error = $return_error(
				new WP_Error(
					...
					sprintf( __( 'The required "%s" parameter is missing.', 'woocommerce' ), 'secret_1' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					/* translators: "%s" is the name of a paramter. It can be either "secret_1" or "state". */
					sprintf( __( 'The required "%s" parameter is malformed.', 'woocommerce' ), 'secret_1' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					'state_missing',
					/* translators: "%s" is the name of a paramter. It can be either "secret_1" or "state". */
					sprintf( __( 'The required "%s" parameter is missing.', 'woocommerce' ), 'state' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					'state_malformed',
					/* translators: "%s" is the name of a paramter. It can be either "secret_1" or "state". */
					sprintf( __( 'The required "%s" parameter is malformed.', 'woocommerce' ), 'state' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					__( 'Verification secrets not found', 'woocommerce' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					__( 'Verification took too long', 'woocommerce' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					__( 'Verification secrets are empty', 'woocommerce' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					__( 'Verification secrets are incomplete', 'woocommerce' ),
					400
				)
			);
			...
			$error = $return_error(
				new WP_Error(
					...
					__( 'Secret mismatch', 'woocommerce' ),
					400
				)
			);
		}
