...
class REST_Connector {
	...
	public function __construct( Manager $connection ) {
		...
		register_rest_route(
			...
			array(
				...
				'args'                => array(
					'isActive' => array(
						'description'       => __( 'Set to false will trigger the site to disconnect.', 'woocommerce' ),
						'validate_callback' => function ( $value ) {
							if ( false !== $value ) {
								return new WP_Error(
									'rest_invalid_param',
									__( 'The isActive argument should be set to false.', 'woocommerce' ),
									array( 'status' => 400 )
								);
							}
		...
		register_rest_route(
			...
			array(
				...
				'args'                => array(
					'from'               => array(
						'description' => __( 'Indicates where the registration action was triggered for tracking/segmentation purposes', 'woocommerce' ),
						'type'        => 'string',
					),
					'registration_nonce' => array(
						'description' => __( 'The registration nonce', 'woocommerce' ),
						'type'        => 'string',
						'required'    => true,
					),
					'redirect_uri'       => array(
						'description' => __( 'URI of the admin page where the user should be redirected after connection flow', 'woocommerce' ),
						'type'        => 'string',
					),
					'plugin_slug'        => array(
						'description' => __( 'Indicates from what plugin the request is coming from', 'woocommerce' ),
						'type'        => 'string',
					),
				),
			)
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'user_token'          => array(
							'description' => __( 'New user token', 'woocommerce' ),
							'type'        => 'string',
							'required'    => true,
						),
						'is_connection_owner' => array(
							'description' => __( 'Is connection owner', 'woocommerce' ),
							'type'        => 'boolean',
						),
					),
				),
		...
		register_rest_route(
			...
			array(
				...
				'args'                => array(
					'owner' => array(
						'description' => __( 'New owner', 'woocommerce' ),
						'type'        => 'integer',
						'required'    => true,
					),
				),
	...
	public function connection_register( $request ) {
		if ( ! wp_verify_nonce( $request->get_param( 'registration_nonce' ), 'jetpack-registration-nonce' ) ) {
			return new WP_Error( 'invalid_nonce', __( 'Unable to verify your request.', 'woocommerce' ), array( 'status' => 403 ) );
		}
	...
	public static function update_user_token( $request ) {
		...
		if ( count( $token_parts ) !== 3 || ! (int) $token_parts[2] || ! ctype_digit( $token_parts[2] ) ) {
			return new WP_Error( 'invalid_argument_user_token', esc_html__( 'Invalid user token is provided', 'woocommerce' ) );
		}

		$user_id = (int) $token_parts[2];

		if ( false === get_userdata( $user_id ) ) {
			return new WP_Error( 'invalid_argument_user_id', esc_html__( 'Invalid user id is provided', 'woocommerce' ) );
		}

		$connection = new Manager();

		if ( ! $connection->is_connected() ) {
			return new WP_Error( 'site_not_connected', esc_html__( 'Site is not connected', 'woocommerce' ) );
		}
	...
	public static function disconnect_site() {
		...
		return new WP_Error(
			'disconnect_failed',
			esc_html__( 'Failed to disconnect the site as it appears already disconnected.', 'woocommerce' ),
			array( 'status' => 400 )
		);
	}
