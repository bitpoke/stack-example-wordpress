		...
		try {
			...
			if ( ! isset( $_SERVER['REQUEST_METHOD'] ) ) {
				$this->rest_authentication_status = new \WP_Error(
					...
					__( 'The request method is missing.', 'woocommerce' ),
					array( 'status' => 400 )
				);
				return null;
			}
			...
			if ( 'GET' !== $_SERVER['REQUEST_METHOD'] && 'POST' !== $_SERVER['REQUEST_METHOD'] ) {
				$this->rest_authentication_status = new \WP_Error(
					'rest_invalid_request',
					__( 'This request method is not supported.', 'woocommerce' ),
					array( 'status' => 400 )
				);
				return null;
			}
			if ( 'POST' !== $_SERVER['REQUEST_METHOD'] && ! empty( file_get_contents( 'php://input' ) ) ) {
				$this->rest_authentication_status = new \WP_Error(
					'rest_invalid_request',
					__( 'This request method does not support body parameters.', 'woocommerce' ),
					array( 'status' => 400 )
				);
				return null;
			}
			...
			$this->rest_authentication_status = new \WP_Error(
				...
				__( 'The request is not signed correctly.', 'woocommerce' ),
				array( 'status' => 400 )
			);
			return null;
		} finally {
