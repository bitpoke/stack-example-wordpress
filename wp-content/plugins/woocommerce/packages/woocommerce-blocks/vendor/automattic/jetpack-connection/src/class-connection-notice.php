...
class Connection_Notice {
	...
	public function delete_user_update_connection_owner_notice() {
		...
		echo '<h2>' . esc_html__( 'Important notice about your Jetpack connection:', 'woocommerce' ) . '</h2>';
		echo '<p>' . sprintf(
			/* translators: WordPress User, if available. */
			esc_html__( 'Warning! You are about to delete the Jetpack connection owner (%s) for this site, which may cause some of your Jetpack features to stop working.', 'woocommerce' ),
			esc_html( $user )
		) . '</p>';

		if ( ! empty( $connected_admins ) && count( $connected_admins ) > 1 ) {
			echo '<form id="jp-switch-connection-owner" action="" method="post">';
			echo "<label for='owner'>" . esc_html__( 'You can choose to transfer connection ownership to one of these already-connected admins:', 'woocommerce' ) . ' </label>';
			...
			echo '<p>';
			submit_button( esc_html__( 'Set new connection owner', 'woocommerce' ), 'primary', 'jp-switch-connection-owner-submit', false );
			echo '</p>';
				...
				( function() {
					...
					switchOwnerButton.addEventListener( 'submit', function ( e ) {
						...
						const handleAPIError = ( message ) => {
							...
							results.innerHTML = message || "<?php esc_html_e( 'Something went wrong. Please try again.', 'woocommerce' ); ?>";
						}
						...
						fetch(
							...
							.then( data => {
								if ( data.hasOwnProperty( 'code' ) && data.code === 'success' ) {
									...
									results.innerHTML = <?php echo wp_json_encode( esc_html__( 'Success!', 'woocommerce' ), JSON_HEX_TAG | JSON_HEX_AMP ); ?>;
									setTimeout(function () {
			...
			echo '<p>' . esc_html__( 'Every Jetpack site needs at least one connected admin for the features to work properly. Please connect to your WordPress.com account via the button below. Once you connect, you may refresh this page to see an option to change the connection owner.', 'woocommerce' ) . '</p>';
			...
			echo "<a href='" . esc_url( $connect_url ) . "' target='_blank' rel='noopener noreferrer' class='button-primary'>" . esc_html__( 'Connect to WordPress.com', 'woocommerce' ) . '</a>';
		}
		...
		printf(
			...
			/* translators: URL to Jetpack support doc regarding the primary user. */
				__( "<a href='%s' target='_blank' rel='noopener noreferrer'>Learn more</a> about the connection owner and what will break if you do not have one.", 'woocommerce' ),
				array(
					'a' => array(
						'href'   => true,
						'target' => true,
		...
		printf(
			...
			/* translators: URL to contact Jetpack support. */
				__( 'As always, feel free to <a href="%s" target="_blank" rel="noopener noreferrer">contact our support team</a> if you have any questions.', 'woocommerce' ),
				array(
					'a' => array(
						'href'   => true,
						'target' => true,
