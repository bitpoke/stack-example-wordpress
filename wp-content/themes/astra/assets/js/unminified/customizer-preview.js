...
// Breakpoints from theme settings — populated via astraCustomizer localized data.
// Defaults match astra_get_tablet_breakpoint() (921) and astra_get_mobile_breakpoint() (544).
const astraTabletBreakpoint = ( astraCustomizer && astraCustomizer.tablet_breakpoint ) ? parseInt( astraCustomizer.tablet_breakpoint, 10 ) : 921;
const astraMobileBreakpoint = ( astraCustomizer && astraCustomizer.mobile_breakpoint ) ? parseInt( astraCustomizer.mobile_breakpoint, 10 ) : 544;
		...
		value.bind( function( value ) {
			if ( value.desktop || value.mobile || value.tablet ) {
				...
				astra_update_style(
					control + '-' + addon,
					selector + '	{ ' + DeskVal + ' }'
					+ '@media (max-width: ' + astraTabletBreakpoint + 'px) {' + selector + '	{ ' + TabletFontVal + ' } }'
					+ '@media (max-width: ' + astraMobileBreakpoint + 'px) {' + selector + '	{ ' + MobileVal + ' } }'
				);

			} else {
				astra_clear_style( control + '-' + addon );
			}
 ...
 */
function astra_responsive_font_size( control, selector, dispatchCustomEvent = false, styleGuideSelector = '' ) {
	...
	wp.customize( control, function( value ) {
		value.bind( function( value ) {
			...
			if ( value.desktop || value.mobile || value.tablet ) {
				...
				astra_update_style(
					control + '-' + css_property,
					selector + '	{ ' + fontSize + ' }'
					+ '@media (max-width: ' + astraTabletBreakpoint + 'px) {' + selector + '	{ ' + tabletFontSize + ' } }'
					+ '@media (max-width: ' + astraMobileBreakpoint + 'px) {' + selector + '	{ ' + mobileFontSize + ' } }'
				);
				...
				astra_clear_style( control + '-font-size' );
			}
	...
	wp.customize( 'astra-settings[' + control + ']', function( value ) {
		value.bind( function( data ) {
			if ( data ) {
				...
				astra_update_style( control, dynamicStyle );

			} else {
				astra_clear_style( control );
			}
		} );
	} );
}
	...
	wp.customize( control, function( value ) {
		value.bind( function( value ) {
			...
			if ( value.desktop.top || value.desktop.right || value.desktop.bottom || value.desktop.left || value.tablet.top || value.tablet.right || value.tablet.bottom || value.tablet.left || value.mobile.top || value.mobile.right || value.mobile.bottom || value.mobile.left ) {
				...
				astra_update_style(
					control + '-' + spacingType + '-' + sidesString,
					selector + '	{ ' + desktopPadding +' }'
					+ '@media (max-width: ' + astraTabletBreakpoint + 'px) {' + selector + '	{ ' + tabletPadding + ' } }'
					+ '@media (max-width: ' + astraMobileBreakpoint + 'px) {' + selector + '	{ ' + mobilePadding + ' } }'
				);

			} else {
				wp.customize.preview.send( 'refresh' );
				astra_clear_style( control + '-' + spacingType + '-' + sidesString );
			}
	...
	wp.customize( control, function( value ) {
		value.bind( function( size ) {
			...
			if ( size ) {
				...
				astra_update_style( control, selector + '	{ ' + fontSize + ' }' );
	...
	wp.customize( control, function( value ) {
		value.bind( function( new_value ) {
			...
			if ( new_value || 0 === new_value ) {
				...
				if( 'unset' === new_value ){
					astra_clear_style( control + '-' + css_property );
				} else {
					astra_update_style(
						control + '-' + css_property,
...
/**
 * Update an existing <style> element in-place, or create it if absent.
 * Avoids remove+append DOM thrash on every customizer slider/change event.
 *
 * @param {string} id  The style element id (already sanitized — no brackets).
 * @param {string} css The CSS text to set.
 */
function astra_update_style( id, css ) {
...
/**
 * Remove a dynamic <style> element if it exists.
 *
 * @param {string} id The style element id (already sanitized — no brackets).
 */
function astra_clear_style( id ) {
	const el = document.getElementById( id );
	if ( el ) {
		el.remove();
	}
...
function astra_add_dynamic_css( control, style ) {
	control = control.replace( '[', '-' );
	control = control.replace( ']', '' );
	astra_update_style( control, style );
}
...
function astra_background_obj_css( wp_customize, bg_obj, ctrl_name, style ) {
	...
	if( '' === bg_color && '' === bg_img ) {
		astra_clear_style( ctrl_name.replace( '[', '-' ).replace( ']', '' ) );
	}else{
...
*/
function astra_generate_outside_font_family_css( control, selector, dispatchCustomEvent = false, styleGuideSelector = '') {
	wp.customize( control, function (value) {
    ...
    wp.customize( 'astra-settings[' + section + '-padding]', function( value ) {
        value.bind( function( padding ) {
            ...
            if(
                ...
                dynamicStyle +=  '@media (max-width: ' + astraTabletBreakpoint + 'px) {';
                dynamicStyle += selector + ' {';
                ...
                dynamicStyle +=  '@media (max-width: ' + astraMobileBreakpoint + 'px) {';
                dynamicStyle += selector + ' {';
...
function astra_apply_responsive_background_css( control, selector, device, singleColorSelector, addon ) {
	wp.customize( control, function( value ) {
		value.bind( function( bg_obj ) {
			...
			const styleId = control + '-' + device + '-' + addon;
			let dynamicCss;
			if ( 'desktop' == device ) {
				dynamicCss = selector + '	{ ' + gen_bg_css + ' }';
			} else if ( 'tablet' == device ) {
				dynamicCss = '@media (max-width: ' + astraTabletBreakpoint + 'px) {' + selector + '	{ ' + gen_bg_css + ' } }';
			} else if ( 'mobile' == device ) {
				dynamicCss = '@media (max-width: ' + astraMobileBreakpoint + 'px) {' + selector + '	{ ' + gen_bg_css + ' } }';
			} else {
				return;
			}
			astra_update_style( styleId, dynamicCss );
		});
	});
}
...
( function( $ ) {
	...
	wp.customize( 'astra-settings[ast-header-responsive-logo-width]', function( setting ) {
		setting.bind( function( logo_width ) {
			if ( logo_width['desktop'] != '' || logo_width['tablet'] != '' || logo_width['mobile'] != '' ) {
				...
				@media( max-width: ${astraTabletBreakpoint}px ) {
				...
				@media( max-width: ${astraMobileBreakpoint}px ) {
					#masthead .site-logo-img .custom-logo-link img, .ast-sg-logo-section .site-logo-img .custom-logo-link img {
				...
				var mobileLogoStyle = '.ast-header-break-point #masthead .site-logo-img .custom-mobile-logo-link img, .ast-header-break-point .ast-sg-logo-section .site-logo-img .custom-mobile-logo-link img { max-width: ' + logo_width['tablet'] + 'px; } @media( max-width: ' + astraTabletBreakpoint + 'px ) { .ast-header-break-point #masthead .site-logo-img .custom-mobile-logo-link img, .ast-header-break-point .ast-sg-logo-section .site-logo-img .custom-mobile-logo-link img { max-width: ' + logo_width['tablet'] + 'px; }  @media( max-width: ' + astraMobileBreakpoint + 'px ) { .ast-header-break-point #masthead .site-logo-img .custom-mobile-logo-link img { max-width: ' + logo_width['mobile'] + 'px; }';
				astra_add_dynamic_css( 'mobile-header-logo-width', mobileLogoStyle );

				/**
				 * Logo SVG Icon Stylings.
				 */
				var logoSVGIconStyles = '#masthead .ast-logo-svg-icon svg, .ast-sg-logo-section .ast-logo-svg-icon svg { width: ' + logo_width['desktop'] + 'px; } @media( max-width: ' + astraTabletBreakpoint + 'px ) { #masthead .ast-logo-svg-icon svg, .ast-sg-logo-section .ast-logo-svg-icon svg { width: ' + logo_width['tablet'] + 'px; } #masthead .ast-logo-svg-icon svg, .ast-sg-logo-section .ast-logo-svg-icon svg { width: ' + logo_width['tablet'] + 'px; } } @media( max-width: ' + astraMobileBreakpoint + 'px ) { #masthead .ast-logo-svg-icon svg, .ast-sg-logo-section .ast-logo-svg-icon svg { width: ' + logo_width['mobile'] + 'px; }' + '#masthead .ast-logo-svg-icon svg, .ast-sg-logo-section .ast-logo-svg-icon svg { width: ' + logo_width['mobile'] + 'px; } }';
	...
	wp.customize( 'astra-settings[logo-svg-site-title-gap]', function( setting ) {
		setting.bind( function( logoGap ) {
			if ( logoGap['desktop'] != '' || logoGap['tablet'] != '' || logoGap['mobile'] != '' ) {
				...
				astra_add_dynamic_css( 'logo-svg-site-title-gap', dynamicStyle );
			} else {
				wp.customize.preview.send( 'refresh' );
			}
		} );
	...
	wp.customize( 'astra-settings[display-site-title-responsive]', function( setting ) {
		setting.bind( function( logo_visibility ) {
			...
			var dynamicStyle = '.ast-site-title-wrap .site-title { display: ' + desktopTitleVisibility + ';} @media( max-width: ' + astraTabletBreakpoint + 'px) { .ast-site-title-wrap .site-title { display: ' + tabletTitleVisibility + ';} } @media( max-width: ' + astraMobileBreakpoint + 'px) { .ast-site-title-wrap .site-title { display: ' + mobileTitleVisibility + ';} }';
	...
	wp.customize( 'astra-settings[display-site-tagline-responsive]', function( setting ) {
		setting.bind( function( tagline_visibility ) {
			...
			var dynamicStyle = '.ast-site-title-wrap .site-description { display: ' + desktopTaglineVisibility + ';} @media( max-width: ' + astraTabletBreakpoint + 'px) { .ast-site-title-wrap .site-description { display: ' + tabletTaglineVisibility + ';} } @media( max-width: ' + astraMobileBreakpoint + 'px) { .ast-site-title-wrap .site-description { display: ' + mobileTaglineVisibility + ';} }';
	...
	wp.customize( 'astra-settings[blog-max-width]', function( setting ) {
		setting.bind( function( width ) {
			...
			var dynamicStyle = '@media all and ( min-width: ' + ( astraTabletBreakpoint + 1 ) + 'px ) {';
	...
	wp.customize( 'astra-settings[site-sidebar-width]', function( setting ) {
		setting.bind( function( width ) {
			...
			if ( ! jQuery( 'body' ).hasClass( 'ast-no-sidebar' ) && width >= 15 && width <= 50 ) {
				...
				var dynamicStyle = '@media (min-width: ' + ( astraTabletBreakpoint + 1 ) + 'px) {';
	 ...
	 */
	astra_css( 'astra-settings[footer-sml-divider]', 'border-top-width', '.ast-small-footer', 'px' );

	/**
	 * Footer Widget Top Border
	 */
	astra_css( 'astra-settings[footer-adv-border-width]', 'border-top-width', '.footer-adv .footer-adv-overlay', 'px' );
	astra_css( 'astra-settings[footer-adv-border-color]', 'border-top-color', '.footer-adv .footer-adv-overlay' );


	/**
	 * Small Footer Top Border Color
	 */
	astra_css( 'astra-settings[footer-sml-divider-color]', 'border-top-color', '.ast-small-footer' );
	...
	wp.customize( 'astra-settings[button-radius-fields]', function( setting ) {
		setting.bind( function( border ) {
			...
			lmsButtonSelectors + search_button_selector + woo_btn_normal_sector + lifterLMSBtnSelector;
			...
			dynamicStyle += '@media (max-width: ' + astraTabletBreakpoint + 'px) { ' + globalSelector + '{ border-top-left-radius :' + border['tablet']['top'] + border['tablet-unit']
			...
			dynamicStyle += '@media (max-width: ' + astraMobileBreakpoint + 'px) { ' + globalSelector + '{ border-top-left-radius :' + border['mobile']['top'] + border['mobile-unit']
	...
	astra_responsive_font_size( 'astra-settings[font-size-site-tagline]', '.ast-sg-logo-section .site-description, .site-header .site-description' );
	...
	wp.customize( 'astra-settings[post-card-border-radius]', function( setting ) {
		setting.bind( function( border ) {
			...
			let blog_layout = (typeof ( wp.customize._value['astra-settings[blog-layout]'] ) != 'undefined') ? wp.customize._value['astra-settings[blog-layout]']._value : 'blog-layout-1';
			...
			dynamicStyle += '@media (max-width: ' + astraTabletBreakpoint + 'px) { ' + globalSelector + '{ border-top-left-radius :' + tabletTop + border['tablet-unit']
			...
			dynamicStyle += '@media (max-width: ' + astraMobileBreakpoint + 'px) { ' + globalSelector + '{ border-top-left-radius :' + mobileTop + border['mobile-unit']
	...
	if (true == astraCustomizer.includeAnchorsInHeadindsCss) {
		astra_responsive_font_size('astra-settings[font-size-h1]', 'h1, .entry-content :where(h1), .entry-content :where(h1) a', true, '.ast-sg-typo-field[data-for="h1"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h2]', 'h2, .entry-content :where(h2), .entry-content :where(h2) a', true, '.ast-sg-typo-field[data-for="h2"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h3]', 'h3, .entry-content :where(h3), .entry-content :where(h3) a', true, '.ast-sg-typo-field[data-for="h3"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h4]', 'h4, .entry-content :where(h4), .entry-content :where(h4) a', true, '.ast-sg-typo-field[data-for="h4"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h5]', 'h5, .entry-content :where(h5), .entry-content :where(h5) a', true, '.ast-sg-typo-field[data-for="h5"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h6]', 'h6, .entry-content :where(h6), .entry-content :where(h6) a', true, '.ast-sg-typo-field[data-for="h6"] .ast-sg-font-size');
	} else {
		astra_responsive_font_size('astra-settings[font-size-h1]', 'h1, .entry-content :where(h1)', true, '.ast-sg-typo-field[data-for="h1"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h2]', 'h2, .entry-content :where(h2)', true, '.ast-sg-typo-field[data-for="h2"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h3]', 'h3, .entry-content :where(h3)', true, '.ast-sg-typo-field[data-for="h3"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h4]', 'h4, .entry-content :where(h4)', true, '.ast-sg-typo-field[data-for="h4"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h5]', 'h5, .entry-content :where(h5)', true, '.ast-sg-typo-field[data-for="h5"] .ast-sg-font-size');
		astra_responsive_font_size('astra-settings[font-size-h6]', 'h6, .entry-content :where(h6)', true, '.ast-sg-typo-field[data-for="h6"] .ast-sg-font-size');
	}
	...
	if ( astraCustomizer.page_builder_button_style_css ) {
		if (true == astraCustomizer.includeAnchorsInHeadindsCss) {
			...
			astra_css('astra-settings[headings-line-height]', 'line-height', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .entry-content :where(h1, h2, h3, h4, h5, h6) a, .site-title, .site-title a');
		} else {
			if ( 'color-typo' == astraCustomizer.elementor_default_color_font_setting || 'typo' == astraCustomizer.elementor_default_color_font_setting ) {
				astra_css('astra-settings[headings-line-height]', 'line-height', '.elementor-widget-heading h1.elementor-heading-title, .elementor-widget-heading h2.elementor-heading-title, .elementor-widget-heading h3.elementor-heading-title, .elementor-widget-heading h4.elementor-heading-title, .elementor-widget-heading h5.elementor-heading-title, .elementor-widget-heading h6.elementor-heading-title');
			}
			astra_css('astra-settings[headings-line-height]', 'line-height', 'h1, .entry-content :where(h1), h2, .entry-content :where(h2), h3, .entry-content :where(h3), h4, .entry-content :where(h4), h5, .entry-content :where(h5), h6, .entry-content :where(h6), .site-title, .site-title a');
		}
	} else {
		if (true == astraCustomizer.includeAnchorsInHeadindsCss) {
			astra_css('astra-settings[headings-line-height]', 'line-height', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .entry-content :where(h1, h2, h3, h4, h5, h6) a, .site-title, .site-title a');
		} else {
			astra_css('astra-settings[headings-line-height]', 'line-height', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .site-title, .site-title a');
		}
	}

	// Check if anchors should be loaded in the CSS for headings.
	if (true == astraCustomizer.includeAnchorsInHeadindsCss) {
		astra_generate_outside_font_family_css('astra-settings[headings-font-family]', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .entry-content :where(h1, h2, h3, h4, h5, h6) a, .site-title, .site-title a');
		astra_generate_font_weight_css( 'astra-settings[headings-font-family]', 'astra-settings[headings-font-weight]', 'font-weight', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .entry-content :where(h1, h2, h3, h4, h5, h6) a, .site-title, .site-title a' );
		astra_font_extras_css( 'headings-font-extras', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .entry-content :where(h1, h2, h3, h4, h5, h6) a, .site-title, .site-title a' );

	} else {
		astra_generate_outside_font_family_css('astra-settings[headings-font-family]', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .site-title, .site-title a');
		astra_generate_font_weight_css( 'astra-settings[headings-font-family]', 'astra-settings[headings-font-weight]', 'font-weight', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .site-title, .site-title a' );
		astra_font_extras_css( 'headings-font-extras', 'h1, h2, h3, h4, h5, h6, .entry-content :where(h1, h2, h3, h4, h5, h6), .site-title, .site-title a' );
	}
		...
		value.bind( function( border ) {
			...
			if( '' != border.top || '' != border.right || '' != border.bottom || '' != border.left ) {
				...
				var dynamicStyle = '.ast-desktop .main-header-menu.submenu-with-border .sub-menu';
					...
					dynamicStyle += '@media (min-width: ' + ( astraTabletBreakpoint + 1 ) + 'px){';
	...
	if ( astraCustomizer.page_builder_button_style_css ) {
		if ( 'color-typo' == astraCustomizer.elementor_default_color_font_setting || 'color' == astraCustomizer.elementor_default_color_font_setting || 'font' == astraCustomizer.elementor_default_color_font_setting ) {
			ele_border_radius_selector = ', .elementor-widget-button .elementor-button.elementor-size-sm, .elementor-widget-button .elementor-button.elementor-size-xs, .elementor-widget-button .elementor-button.elementor-size-md, .elementor-widget-button .elementor-button.elementor-size-lg, .elementor-widget-button .elementor-button.elementor-size-xl, .elementor-widget-button .elementor-button';
			ele_border_width_selector = ', .elementor-widget-button .elementor-button, .elementor-widget-button .elementor-button:visited';
			ele_padding_selector = ', .elementor-widget-button .elementor-button.elementor-size-sm, .elementor-widget-button .elementor-button.elementor-size-xs, .elementor-widget-button .elementor-button.elementor-size-md, .elementor-widget-button .elementor-button.elementor-size-lg, .elementor-widget-button .elementor-button.elementor-size-xl, .elementor-widget-button .elementor-button';
		}
	}
	...
	wp.customize( 'astra-settings[theme-button-border-group-border-size]', function( value ) {
		value.bind( function( border ) {
			...
			var dynamicStyle = '.menu-toggle, button, .ast-button, .ast-custom-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .wp-block-button .wp-block-button__link' + astraCustomizer.v4_2_2_core_form_btns_styling + ele_border_width_selector + search_button_selector + woo_btn_normal_sector + lifterLMSBtnSelector;
	...
	astra_generate_outside_font_family_css( 'astra-settings[font-family-site-tagline]', '.ast-sg-logo-section .site-description, .site-header .site-description' );

	// Site Tagline - Font Weight
	astra_generate_font_weight_css( 'astra-settings[font-family-site-tagline]', 'astra-settings[font-weight-site-tagline]', 'font-weight', '.ast-sg-logo-section .site-description, .site-header .site-description' );

	// Site Tagline - Font Size
	astra_responsive_font_size( 'astra-settings[font-size-site-tagline]', '.ast-sg-logo-section .site-description, .site-header .site-description' );

	// Site Tagline - Line Height
	astra_css( 'astra-settings[line-height-site-tagline]', 'line-height', '.ast-sg-logo-section .site-description, .site-header .site-description' );
	...
	if ( astraCustomizer.page_builder_button_style_css ) {
		...
		if ( 'color-typo' == astraCustomizer.elementor_default_color_font_setting || 'color' == astraCustomizer.elementor_default_color_font_setting ) {
			...
			btn_color_ele = ',.elementor-widget-button .elementor-button, .elementor-widget-button .elementor-button:visited';

			// Theme Button - Background Color
			btn_bg_color_ele = ',.elementor-widget-button .elementor-button, .elementor-widget-button .elementor-button:visited';

			// Theme Button - Text Hover Color
			btn_h_color_ele = ',.elementor-widget-button .elementor-button:hover, .elementor-widget-button .elementor-button:focus';

			// Theme Button - Background Hover Color
			btn_bg_h_color_ele = ',.elementor-widget-button .elementor-button:hover, .elementor-widget-button .elementor-button:focus';

			// Theme Button - Border Color
			btn_border_color_ele = ', .elementor-widget-button .elementor-button, .elementor-widget-button .elementor-button:visited';

			// Theme Button - Border Hover Color
			btn_border_h_color_ele = ',.elementor-widget-button .elementor-button:hover, .elementor-widget-button .elementor-button:focus';
		}

		var btnSelector = '.menu-toggle, button, .ast-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .wp-block-button:not(.is-style-outline) .wp-block-button__link, .ast-custom-button' + astraCustomizer.v4_2_2_core_form_btns_styling + btn_bg_color_ele + search_button_selector + woo_btn_normal_sector + lifterLMSBtnSelector;
		...
		var btnSelector = '.menu-toggle, button, .ast-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .ast-custom-button' + search_button_selector + woo_btn_normal_sector + lifterLMSBtnSelector;

		astraHandleButtonPresetPreview( btnSelector );

		// Theme Button - Border Color
		astra_css( 'astra-settings[button-bg-color]', 'border-color', '.menu-toggle, button, .ast-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .ast-custom-button' + search_button_selector + lifterLMSBtnSelector );

		// Theme Button - Text Hover Color
		astra_css( 'astra-settings[button-h-color]', 'color', 'button:focus, .menu-toggle:hover, button:hover, .ast-button:hover, .button:hover, input[type=reset]:hover, input[type=reset]:focus, input#submit:hover, input#submit:focus, input[type="button"]:hover, input[type="button"]:focus, input[type="submit"]:hover, input[type="submit"]:focus, .ast-custom-button:hover, .ast-custom-button:focus' + search_button_hover_selector + lifterLMSBtnHSelector );

		// Theme Button - Background Hover Color
		astra_css( 'astra-settings[button-bg-h-color]', 'background-color', 'button:focus, .menu-toggle:hover, button:hover, .ast-button:hover, .button:hover, input[type=reset]:hover, input[type=reset]:focus, input#submit:hover, input#submit:focus, input[type="button"]:hover, input[type="button"]:focus, input[type="submit"]:hover, input[type="submit"]:focus, .ast-custom-button:hover, .ast-custom-button:focus' + search_button_hover_selector + lifterLMSBtnHSelector );

		astra_responsive_spacing( 'astra-settings[theme-button-padding]','.menu-toggle, button, .ast-button, .ast-custom-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .woocommerce a.button, .woocommerce button.button, .woocommerce .product a.button, .woocommerce .woocommerce-message a.button, .woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce input.button,.woocommerce input.button:disabled, .woocommerce input.button:disabled[disabled]' + astraCustomizer.v4_2_2_core_form_btns_styling + search_button_selector + lifterLMSBtnSelector, 'padding', [ 'top', 'bottom' ] );
		astra_responsive_spacing( 'astra-settings[theme-button-padding]','.menu-toggle, button, .ast-button, .ast-custom-button, .button, input#submit, input[type="button"], input[type="submit"], input[type="reset"], .woocommerce a.button, .woocommerce button.button, .woocommerce .product a.button, .woocommerce .woocommerce-message a.button, .woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt, .woocommerce input.button,.woocommerce input.button:disabled, .woocommerce input.button:disabled[disabled]' + astraCustomizer.v4_2_2_core_form_btns_styling + search_button_selector + lifterLMSBtnSelector, 'padding', [ 'left', 'right' ] );
	}
		...
		value.bind(function (value) {
			if ( value.desktop || value.mobile || value.tablet ) {
				...
				astra_update_style(
					control + '-' + css_property,
					selector + '	{ ' + fontSize + ' }'
					+ '@media (max-width: ' + astraTabletBreakpoint + 'px) {.ast-header-break-point ' + selector + ' { ' + tabletFontSize + ' } }'
					+ '@media (max-width: ' + astraMobileBreakpoint + 'px) {.ast-header-break-point ' + selector + ' { ' + mobileFontSize + ' } }'
				);
	...
	function astraHandleButtonPresetPreview( btnSelector ) {
			...
			setting.bind( function( value ) {
				...
				if( 'button_04' === value || 'button_05' === value || 'button_06' === value ) {
					...
					if( '' === buttonTextColor && '' !== buttonBGColor ) {
						astra_update_style(
							'astra-settings-button-outline-preset-color',
					...
					astra_update_style(
						'astra-settings-button-preset-style-background-color',
						btnSelector + '	{ background: transparent }'
					);
				}
				else {

					astra_clear_style( 'astra-settings-button-bg-color-background-color' );
					astra_clear_style( 'astra-settings-button-outline-preset-color' );

					if( '' === buttonTextColor && '' === buttonBGColor ) {

						astra_update_style(
							'astra-settings-button-bg-color-background-color',
					...
					if( '' === buttonTextColor ) {
						...
						astra_clear_style( 'astra-settings-button-color-color' );
					}
				}
		...
		wp.customize( 'astra-settings[secondary-button-preset-style]', function( setting ) {
			...
			setting.bind( function( value ) {
				...
				if( 'button_04' === value || 'button_05' === value || 'button_06' === value ) {
					...
					if( '' === buttonBorderColor ) {
						astra_update_style(
							'astra-settings-secondary-theme-button-border-group-border-color',
					...
					if( '' === buttonTextColor && '' !== buttonBGColor ) {
						astra_update_style(
							'astra-settings-secondary-button-outline-preset-color',
					...
					astra_update_style(
						'astra-settings-secondary-button-preset-style-background-color',
						btnBgColorSelector + '	{ background: transparent }'
					);
				} else {

					astra_clear_style( 'astra-settings-secondary-button-bg-color-background-color' );
					astra_clear_style( 'astra-settings-secondary-button-outline-preset-color' );

					if( '' === buttonTextColor && '' === buttonBGColor ) {

						astra_update_style(
							'astra-settings-secondary-button-bg-color-background-color',
					...
					if( '' === buttonTextColor ) {
						...
						astra_clear_style( 'astra-settings-secondary-button-color-color' );
					}
			...
			setting.bind( function( value ) {
				if( '' === value ) {
					...
					if( 'button_04' === buttonPreset || 'button_05' === buttonPreset || 'button_06' === buttonPreset ) {
						...
						astra_clear_style( 'astra-settings-button-outline-color' );

						astra_update_style(
							'astra-settings-button-color-color',
							btnSelector + '	{ color: ' + buttonBGColor + ' }'
						);
					}
					 else {
			...
			setting.bind( function( value ) {
				if( '' === value ) {
					...
					if( 'button_04' === buttonPreset || 'button_05' === buttonPreset || 'button_06' === buttonPreset ) {
						...
						astra_update_style(
							'astra-settings-secondary-button-color-color',
							btnSecondaryColorSelector + '	{ color: ' + themeColor + ' }'
						);
					}
		...
		// Bail early if WooCommerce is not active — skip all WooCommerce customizer bindings.
		if ( ! astraCustomizer.is_woo_active ) {
			return;
		}
